...
const Register = () => {
  ...
  const { RegisterAccount, setUser, loginAccount_Google } = use(AuthContext);
  ...
  //  md:w-150 md:h-13 
  // md:right-3 md:top-1/2 md:transform md:-translate-y-1/2
  // w-80 h-150 md:w-200 md:h-200
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-4 mt-10">
      <div className="card bg-base-100  shrink-0 shadow-xl max-md:h-165">
        <div className="card-body max-md:w-80 max-md:h-150">
          ...
          <form onSubmit={handleRegistation} className="flex justify-center items-center mt-2">
            <fieldset className="fieldset gap-3">
              {/* Name */}
              <label className="label text-sm md:text-xl font-bold">Name</label>
              <input type="text" name='name' required className="input w-70 h-10 outline-none bg-gray-100 md:w-150 md:h-13 " placeholder="Name" />
              {/* Photo URL */}
              ...
              <label className="label text-sm md:text-xl flex items-center gap-2">
          ...
          <Link to={'/auth/login'} className='text-xl max-md:text-sm max-md:mt-3 text-center'>Already have An Account ? <span className='text-red-500 link-hover cursor-pointer font-bold'>Login</span></Link>
        </div>
